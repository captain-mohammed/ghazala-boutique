...
- **Model identity is minted, never inferred.** Every new registration gets a **fresh** `modelId` — never adopt an existing one because the name/category/colour happen to match. `autoName` is derived from type + first colour only (`بوت أسود` for any black boot), so two genuinely different products can share it; inferring identity from it silently merged them, mixing photos and prices and inflating quantities (shipped as a real bug, fixed in v0.21.3). To add a colour to an existing model, use that model's «تعديل» or فاتورة الوارد's «لون آخر لنفس الموديل» — both share the number deliberately. Grouping is `modelGroupKey = modelId || name|category`; **SKU is never a grouping key**, so two cards with different SKUs can still land on one card if they share a `modelId`. Editing is scoped to the edited model's own cards only (`ownCards`), never to the whole table.
